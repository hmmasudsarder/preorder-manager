...
} from "@/app/redux/features/preorder/preorder.api";
...
export default function PreorderForm({ preorderId }: PreorderFormProps) {
  ...
  // --- RTK Query Hooks ---
  // কন্ডিশনাল কোয়েরি (শুধু এডিট মোডেই এবং preorderId থাকলে সিঙ্গেল ডাটা ফেচ হবে)
  const { data: singleData, isLoading: isFetchLoading } = useGetSinglePreorderQuery(
    preorderId as string,
    { skip: !isEditMode || !preorderId }
  );
  ...
  // হেল্পার ফাংশন: ISO Date-কে datetime-local ইনপুট ফরম্যাটে রূপান্তর করার জন্য
  const formatYYYYMMDDTHHMM = (dateString?: string) => {
    if (!dateString) return "";
    const d = new Date(dateString);
    if (isNaN(d.getTime())) return "";
    
    // টাইমজোন অফসেট ঠিক রেখে সঠিকভাবে YYYY-MM-DDTHH:mm ফরম্যাট তৈরি করা
    const pad = (num: number) => String(num).padStart(2, "0");
    return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
  };

  // ডাটাবেজ থেকে ডাটা লোড হলে ফর্মে প্রি-ফিল করা
  useEffect(() => {
    if (isEditMode && singleData?.data) {
      ...
      setFormData({
        ...
        startsAt: formatYYYYMMDDTHHMM(item.startsAt),
        endsAt: formatYYYYMMDDTHHMM(item.endsAt),
