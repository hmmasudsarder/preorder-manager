...
export default function PreorderForm() {
  ...
  // ইউআরএল-এর ফোল্ডার নাম [id] হলে params.id হবে, [preorderId] হলে params.preorderId হবে
  const preorderId = Array.isArray(params?.id)
    ? params?.id[0]
    : params?.id;
  ...
  const { data: singleData, isLoading: isFetchLoading } = useGetSinglePreorderQuery(
    preorderId ?? "",
    { skip: !isEditMode } // আইডি ভ্যালিড না হলে এপিআই কল স্কিপ করবে
  );
